...
// Highlight reading from question (streamer draws for viewer's question)
app.post('/api/reading-highlight', authMiddleware, (req, res) => {
  const { channelId, questionId, reading } = req.body;
  if (!reading || !channelId) return res.status(400).json({ error: 'Missing fields' });

  // Mark question as answered if found
